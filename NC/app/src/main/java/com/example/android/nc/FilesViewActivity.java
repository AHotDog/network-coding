...
public class FilesViewActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		requestKey=intent.getIntExtra("key",-1);
		if(requestKey==4)
			Toast.makeText(getApplicationContext(), "选择要打开的文件", Toast.LENGTH_SHORT).show();
		else
		...
		if(!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED))
			path=Environment.getRootDirectory().getPath();//SD卡不可用
		else
			path=Environment.getExternalStorageDirectory().getPath();
		if(requestKey==3)
			path+="/cutfiles/";
		else if(requestKey==4)
			path+="/files/";
		this.FilesListView(path);

		/*使用SimpleAdapter绑定数据与视图*/
	...
	private void FilesListView(String selectedPath){
		...
		//File selectedFile = new File(selectedPath+"/DCIM/Camera");
